      ...
      fetchPage(pageNo)
        ...
        .catch(err => {
          if (n >= retries) {
            ...
            const nextDelay = backoff * Math.pow(2, n - 1);
...
function parseDrawResult(item) {
  ...
  return {
    ...
    front: numbers, // 3个号码均放入front，排列三没有后区
    back: [],
...
async function runIncrementalScrape() {
  console.log('🎰 开始执行体彩排列三历史开奖数据自动同步机制...\n');
  ...
  try {
    ...
    for (pageNo = 1; pageNo <= totalPages; pageNo++) {
      ...
      if (pageNo === 1) {
        ...
        await sleep(100); // 频控保护，排列三全量抓取较快
